// src/components/SaveChatModal.js - Save Chat Dialog

import React, { useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { saveChat, getAllTags } from '../utils/savedChatsUtils';
...
const SaveChatModal = ({
  ...
  const [existingTags] = useState(() => getAllTags());

  const handleSave = async () => {
    setIsSaving(true);

    try {
      const result = saveChat(messages, title.trim() || null, tags);
      ...
      if (result.success) {
        ...
        alert(language === 'ar' ? 'حدث خطأ في حفظ المحادثة' : 'Error saving chat');
      }
    } catch (error) {
        ...
        >
          ...
          <div style={{
            ...
            <h2 style={{
              ...
              {language === 'ar' ? 'حفظ المحادثة' : 'Save Chat'}
            </h2>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <label style={{
              ...
              {language === 'ar' ? 'عنوان المحادثة' : 'Chat Title'}
          ...
          <div style={{ marginBottom: '2rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              ...
              <button
                ...
                style={{
                  padding: '0.5rem 1rem',
                  background: newTag.trim() && !isSaving ? '#667eea' : 'rgba(0, 0, 0, 0.1)',
